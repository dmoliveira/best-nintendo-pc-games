...
test("home shell exposes the reviewed catalog without pretending search exists", () => {
  assert.doesNotMatch(page, /role="search"/);
  assert.doesNotMatch(page, /disabled/);
  assert.doesNotMatch(page, /<input/);
  assert.match(page, /Reviewed catalog is live/);
  assert.match(page, /getCatalogGames/);
  assert.match(page, /href={`\/games\/\$\{game\.slug\}\/`}/);
