...
export function validateGameArtExport({ game, gameHtml, outDir, assetById, expectedBasePath }) {
  ...
  if (sceneAsset) {
    const sceneUrl = publicAssetUrl(sceneAsset.path, expectedBasePath);
    const sceneTag = tagForSource(gameHtml, sceneUrl);
    if (!gameHtml.includes("game-box-stage__editorial-art") || !sceneTag) failures.push("does not render its approved editorial scene art");
    else if (!hasAttribute(sceneTag, "loading", "lazy") || !hasAttribute(sceneTag, "decoding", "async") || !hasAttribute(sceneTag, "fetchpriority", "low")) failures.push("does not defer its approved editorial scene art");
    if (imagePreloaded(gameHtml, sceneUrl)) failures.push("preloads approved editorial scene art instead of deferring it");
  }
