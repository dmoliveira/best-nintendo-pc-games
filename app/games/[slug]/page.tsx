...
import { getCatalogGame, getCatalogGames, getEditorialSignals } from "@/lib/catalog/site-data";
...
export async function generateMetadata({ params }: GamePageProps): Promise<Metadata> {
  ...
  return {
    title: `${game.title} | GameAtlas`,
    description: game.shortDescription,
    alternates: { canonical: url },
    openGraph: { type: "article", title: `${game.title} | GameAtlas`, description: game.shortDescription, url, images: [{ url: site.publicUrl("og-image.png"), width: 1200, height: 630, alt: `GameAtlas — ${game.title}` }] },
  };
}
...
export default async function GamePage({ params }: GamePageProps) {
  ...
  return <div className="site-shell">
    ...
    <main className="game-page" id="main-content">
      ...
      <section className="game-hero" aria-labelledby="game-title"><div className="game-hero-art" aria-hidden="true">{game.emoji}</div><div><p className="eyebrow">{platforms.map((platform) => platform.name).join(" · ")}</p><h1 id="game-title">{game.title}</h1><p className="game-intro">{game.shortDescription}</p><div className="game-meta"><span>{game.release.year}</span>{genres.map((genre) => <span key={genre.id}>{genre.name}</span>)}</div></div></section>

      <div className="game-content-grid"><section className="game-main-column" aria-labelledby="highlights-heading"><div className="detail-section"><p className="eyebrow">Why it is here</p><h2 id="highlights-heading">A useful place to start.</h2><ul className="highlight-list">{game.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></div><div className="detail-section" aria-labelledby="evidence-heading"><p className="eyebrow">Evidence</p><h2 id="evidence-heading">Read the context.</h2>{editorialSignals.map((signal) => <article className="evidence-card" key={`${signal.provider}-${signal.label}`}><span className="evidence-pill">Original editorial</span><h3>{signal.label}</h3><p>{signal.rationale}</p><small>Reviewed by {signal.reviewedBy ?? "GameAtlas editorial review"} · Captured {signal.capturedAt}</small></article>)}</div></section><aside className="game-sidebar" aria-labelledby="resources-heading"><div className="sidebar-card"><p className="eyebrow">At a glance</p><h2>Game details</h2><dl className="detail-list"><div><dt>Platform</dt><dd>{platforms.map((platform) => platform.name).join(", ")}</dd></div><div><dt>Release year</dt><dd>{game.release.year}</dd></div>{game.developer ? <div><dt>Developer</dt><dd>{game.developer}</dd></div> : null}{game.publisher ? <div><dt>Publisher</dt><dd>{game.publisher}</dd></div> : null}</dl></div><div className="sidebar-card"><p className="eyebrow">Go deeper</p><h2 id="resources-heading">Official resources</h2><ul className="resource-list">{game.links.map((link) => <li key={`${link.kind}-${link.url}`}><a href={link.url} target="_blank" rel="noreferrer">{link.label}<span aria-hidden="true">↗</span></a></li>)}</ul></div></aside></div>
    </main>
