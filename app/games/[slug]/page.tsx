    ...
    <main className="game-page" id="main-content">
      ...
      <section className="game-hero game-hero--with-package" aria-labelledby="game-title">
        ...
        <div>
          ...
          <div className="game-status-row">{editorialSignals.length > 0 ? <span className="editorial-badge"><span className="editorial-dot" aria-hidden="true" />{isCatalogMethodEntry ? "GameAtlas catalog entry" : "GameAtlas pick"}</span> : null}{publicSignals.critic ? <a className="score-value" href={publicSignals.critic.url} target="_blank" rel="noreferrer" title={`${publicSignals.critic.provider} · ${publicSignals.critic.detail}`} aria-label={`${publicSignals.critic.label}: ${publicSignals.critic.display}; ${publicSignals.critic.detail}; source ${publicSignals.critic.provider}; opens in a new tab`}><span className="score-link-label">{publicSignals.critic.label}</span><strong>{publicSignals.critic.display}</strong></a> : criticalLink ? <a className="score-link" href={criticalLink.url} target="_blank" rel="noreferrer" aria-label={`Open ${criticalLink.label.replace(/^External\/reference\s+—\s*/, "")} critic score context for ${game.title}; opens in a new tab`}><span className="score-link-label">Critic score</span><span>{criticalLink.label.replace(/^External\/reference\s+—\s*/, "")} <span aria-hidden="true">↗</span></span></a> : null}{publicSignals.sales ? <a className="sales-summary" href={publicSignals.sales.url} target="_blank" rel="noreferrer" title={`${publicSignals.sales.provider} · ${publicSignals.sales.detail}`} aria-label={`${publicSignals.sales.label}: ${publicSignals.sales.display}; ${publicSignals.sales.detail}; source ${publicSignals.sales.provider}; opens in a new tab`}><span className="score-link-label">{publicSignals.sales.label}</span><strong>{publicSignals.sales.display}</strong></a> : null}</div>
